...
export const AddUserApi=async(cred)=>{
  let res=await createUserWithEmailAndPassword (auth,cred.email,cred.password)
   let res2=await addDoc(userRef,{firstname:cred.firstname,laststname:cred.laststname,email:cred.email,})
...
export const AddAdminApi=async(cred)=>{
   ...
   let res2=await addDoc(AdminRef,{name:cred.name,email:cred.email, password:cred.password,designation:cred.designation})
